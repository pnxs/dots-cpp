...
namespace dots
{
	...
	const Container<>& Dispatcher::container(const type::NewStructDescriptor<>& descriptor) const
	{
		return m_containerPool.get(descriptor);
	}

	Container<>& Dispatcher::container(const type::NewStructDescriptor<>& descriptor)
	{
		return m_containerPool.get(descriptor);
	}

	Subscription Dispatcher::subscribe(const type::NewStructDescriptor<>& descriptor, receive_handler_t<>&& handler)
	{
	...
	Subscription Dispatcher::subscribe(const type::NewStructDescriptor<>& descriptor, event_handler_t<>&& handler)
	{
		Subscription subscription{ m_this, descriptor };
