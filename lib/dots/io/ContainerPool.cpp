...
namespace dots
{
    ...
    const Container<>* ContainerPool::find(const type::NewStructDescriptor<>& descriptor) const
    {
		auto it = m_pool.find(&descriptor);
		return it == m_pool.end() ? nullptr : &it->second;
    }

    const Container<>& ContainerPool::get(const type::NewStructDescriptor<>& descriptor, bool insertIfNotExist/* = true*/) const
    {
		if (insertIfNotExist)
        {
    ...
    Container<>* ContainerPool::find(const type::NewStructDescriptor<>& descriptor)
    {
		return const_cast<Container<>*>(std::as_const(*this).find(descriptor));
    }

    Container<>& ContainerPool::get(const type::NewStructDescriptor<>& descriptor, bool insertIfNotExist/* = true*/)
    {
    ...
    auto ContainerPool::remove(const type::NewStructDescriptor<>& descriptor) -> node_t
    {
