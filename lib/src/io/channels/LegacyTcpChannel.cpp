...
namespace dots::io
{
    LegacyTcpChannel::LegacyTcpChannel(key_t key, boost::asio::io_context& ioContext, const Endpoint& endpoint) :
        LegacyTcpChannel(key, ioContext, endpoint.host(), endpoint.port())
    {
        /* do nothing */
    }

    LegacyTcpChannel::LegacyTcpChannel(key_t key, boost::asio::io_context& ioContext, std::string_view host, std::string_view port) :
    ...
    LegacyTcpChannel::LegacyTcpChannel(key_t key, boost::asio::io_context& ioContext, std::string_view host, std::string_view port, std::function<void(const boost::system::error_code& error)> onConnect) :
        LegacyTcpChannel(key, boost::asio::ip::tcp::socket{ ioContext }, nullptr)
    {
    ...
    LegacyTcpChannel::LegacyTcpChannel(key_t key, boost::asio::ip::tcp::socket&& socket_, payload_cache_t* payloadCache) :
