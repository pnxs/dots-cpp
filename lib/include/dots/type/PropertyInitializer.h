...
namespace dots::type
{
    ...
    struct PropertyInitializer
    {
        ...
        template <typename... Args, typename V = value_t, std::enable_if_t<!std::is_arithmetic_v<V> && !std::disjunction_v<is_property<Args>...>, int> = 0>
        PropertyInitializer(Args&&... args) : value(std::forward<Args>(args)...) {}
        template <typename D>
        PropertyInitializer(const Property<value_t, D>& property) : value(*property) {}
