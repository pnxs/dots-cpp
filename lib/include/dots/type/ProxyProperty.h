...
namespace dots::type
{
    ...
    struct ProxyProperty : Property<T, ProxyProperty<T>>
    {
        ...
        bool is() const
        {
            ...
            return &descriptor_t::InitInstance() == &derivedDescriptor().valueDescriptor();
        }
        ...
        const ProxyProperty<U>& to() const
        {
            ...
            if constexpr (Safe)
            {
                if (!is<U>())
                {
                    throw std::logic_error{ std::string{ "type mismatch in safe ProxyProperty conversion: expected '" } + derivedDescriptor().valueDescriptor().name() + "' but got '" + descriptor_t::InitInstance().name() + "'" };
                }
            }
